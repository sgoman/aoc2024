...
const countCorners = (grid, row, col) => [0, 1, 2, 3]
...
const solve = (part, input, visited) => gridCells(input)
    ...
    .reduce((total, group) => total + group[part].length * group[part].reduce((acc, cur) => acc + cur, 0), 0)

const part1 = input => solve(0, parseInput(input), new Set())

const part2 = input => solve(1, parseInput(input), new Set())
